      ...
      if ($new_pass === $confirm_pass) {
        ...
        if (!empty($token) && !empty($email)) {
          try {
            ...
            if ($res = $maincon->query("SELECT * FROM tblstaff WHERE username='" . $decoded["email"] . "' and verify_token='" . $decoded["verify_token"] . "'")) {
              if ($res->num_rows > 0) {
                $new_token = md5(rand()) . "exp";
                $row = $res->fetch_array();
                if ($maincon->query("UPDATE tblstaff SET verify_token='$new_token',pass='$new_pass' WHERE username='" . $decoded["email"] . "' and verify_token='" . $decoded["verify_token"] . "'")) {
